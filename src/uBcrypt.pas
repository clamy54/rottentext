...
{$mode objfpc}{$H+}
{$Q-}{$R-}  // arithmetique modulaire (Blowfish): pas de controle overflow/range

// bcrypt (Provos & Mazieres), vecteurs Openwall/crypt_blowfish.
// Tables Blowfish reprises de fcl-base/blowfish.pp (domaine public).

interface

// ASalt = 16 octets (parametre non type). ACost 4..31.
function BcryptHash(const APassword: RawByteString; ACost: Integer;
  const ASalt): string;

function BcryptParseSalt(const ACrypt: string; out ACost: Integer;
  out ASalt16): Boolean;

function BcryptEncodeSalt(const ASalt16): string;
...
function BF_F(const St: TBFState; x: DWord): DWord; inline;
begin
  Result := ((St.S[0, (x shr 24) and $FF] + St.S[1, (x shr 16) and $FF]) xor
             St.S[2, (x shr 8) and $FF]) + St.S[3, x and $FF];
end;

procedure BF_Encipher(const St: TBFState; var XL, XR: DWord);
var
  i: Integer;
  l, r, t: DWord;
begin
  ...
  t := l; l := r; r := t;     // annule le dernier swap
  r := r xor St.P[16];
  l := l xor St.P[17];
  XL := l; XR := r;
end;

function Stream2Word(P: PByte; Len: Integer; var Cur: Integer): DWord;
var
  i: Integer;
begin
  Result := 0;
  ...
  for i := 0 to 3 do
  begin
    if Cur >= Len then Cur := 0;
    Result := (Result shl 8) or P[Cur];
    Inc(Cur);
  end;
end;

procedure BF_InitState(out St: TBFState);
begin
  Move(bf_P, St.P, SizeOf(bf_P));
  Move(bf_S, St.S, SizeOf(bf_S));
end;

// ADataP = sel ; nil/0 degrade en expand0state.
procedure BF_ExpandKey(var St: TBFState; ADataP: PByte; ADataLen: Integer;
  AKeyP: PByte; AKeyLen: Integer);
var
  i, j, bi, k: Integer;
  dl, dr: DWord;
begin
  j := 0;
  for i := 0 to 17 do
    St.P[i] := St.P[i] xor Stream2Word(AKeyP, AKeyLen, j);

  dl := 0; dr := 0; j := 0;
  i := 0;
  while i < 18 do
  begin
    dl := dl xor Stream2Word(ADataP, ADataLen, j);
    dr := dr xor Stream2Word(ADataP, ADataLen, j);
    BF_Encipher(St, dl, dr);
    St.P[i] := dl;
    St.P[i + 1] := dr;
    Inc(i, 2);
  end;
...
procedure EksBlowfishSetup(ACost: Integer; ASaltP: PByte; ASaltLen: Integer;
  AKeyP: PByte; AKeyLen: Integer; out St: TBFState);
var
  rounds, i: Int64;
begin
  BF_InitState(St);
  BF_ExpandKey(St, ASaltP, ASaltLen, AKeyP, AKeyLen);
  rounds := Int64(1) shl ACost;
  for i := 1 to rounds do
  begin
    BF_ExpandKey(St, nil, 0, AKeyP, AKeyLen);
    BF_ExpandKey(St, nil, 0, ASaltP, ASaltLen);
  end;
end;

function EncB64(const ABuf; ALen: Integer): string;
var
  p: PByte;
  i: Integer;
  c1, c2: Byte;
begin
  ...
  begin
    c1 := p[i]; Inc(i);
    Result := Result + B64[c1 shr 2];
    c1 := (c1 and $03) shl 4;
    if i >= ALen then
    begin
      Result := Result + B64[c1];
      Break;
    end;
    c2 := p[i]; Inc(i);
    ...
    Result := Result + B64[c1];
    Result := Result + B64[c2 and $3F];
  end;
end;

function Char64(c: Char): Integer;
var
  i: Integer;
begin
  Result := -1;
  for i := 0 to 63 do
    if B64[i] = c then Exit(i);
end;

function DecB64(const S: string; AOut: PByte; AOutLen: Integer): Boolean;
var
  p, o: Integer;
  c1, c2, c3, c4: Integer;
begin
  ...
  begin
    ...
    c1 := Char64(S[p]); c2 := Char64(S[p + 1]);
    if (c1 < 0) or (c2 < 0) then Exit;
    AOut[o] := (c1 shl 2) or ((c2 and $30) shr 4); Inc(o);
    if o >= AOutLen then Break;
    if p + 2 > Length(S) then Exit;
    c3 := Char64(S[p + 2]);
    if c3 < 0 then Exit;
    AOut[o] := ((c2 and $0F) shl 4) or ((c3 and $3C) shr 2); Inc(o);
    if o >= AOutLen then Break;
    if p + 3 > Length(S) then Exit;
    c4 := Char64(S[p + 3]);
    if c4 < 0 then Exit;
    AOut[o] := ((c3 and $03) shl 6) or c4; Inc(o);
    p := p + 4;
  end;
  Result := True;
end;
...
function BcryptHash(const APassword: RawByteString; ACost: Integer;
  const ASalt): string;
var
  keyBuf: RawByteString;
  magic: RawByteString;
  St: TBFState;
  cdata: array[0..5] of DWord;
  outb: array[0..23] of Byte;
  i, k, j: Integer;
...
begin
  ...
  j := 0;
  for i := 0 to 5 do
    cdata[i] := Stream2Word(PByte(@magic[1]), 24, j);
  for k := 0 to 63 do
  begin
    i := 0;
    while i < 6 do
    begin
      BF_Encipher(St, cdata[i], cdata[i + 1]);
      Inc(i, 2);
    end;
  end;
  for i := 0 to 5 do
  begin
    outb[i * 4]     := Byte(cdata[i] shr 24);
    outb[i * 4 + 1] := Byte(cdata[i] shr 16);
    outb[i * 4 + 2] := Byte(cdata[i] shr 8);
    outb[i * 4 + 3] := Byte(cdata[i]);
  end;

  saltStr := EncB64(ASalt, 16);
  hashStr := EncB64(outb[0], 23);  // bcrypt encode 23 des 24 octets
  Result := Format('$2y$%.2d$%s%s', [ACost, saltStr, hashStr]);
end;
...
begin
  ...
  Result := DecB64(saltPart, PByte(@ASalt16), 16);
end;

function BcryptEncodeSalt(const ASalt16): string;
begin
  Result := EncB64(ASalt16, 16);
end;
