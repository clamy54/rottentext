...
The Original Code is: SynEditWrappedView (syneditwrappedview.pp), part of the
SynEdit package shipped with Lazarus (components/synedit). Contributors to the
SynEdit and mwEdit projects are listed in SynEdit's Contributors.txt file.

Description of Modifications (MPL 1.1 section 3.3):
  Forked for RottenText from Lazarus 4.8. The upstream GetWrapColumn is private
  and non-virtual, so wrapping could only follow the window width. This fork
  adds FixedWrapColumn (0 = automatic = window width, otherwise wrap at that
  fixed column) to implement View > Word Wrap Column. That is the ONLY change;
  the added lines are tagged with "fork RottenText" comments.
...
See LICENSE_THIRD_PARTIES.md at the root of the RottenText repository.
-------------------------------------------------------------------------------}
unit uWrapView;

{$mode objfpc}{$H+}
{$WARN 5063 off} // l'original est marque experimental, le fork assume
{$WARN 5079 off}
...
type
  ...
  private
    ...
    FFixedWrapColumn: Integer; // fork RottenText: 0 = largeur fenetre
    ...
    procedure SetFixedWrapColumn(AValue: Integer);
public
  ...
  public
    ...
    // fork RottenText: colonne de wrap fixe (0 = automatique, largeur fenetre)
    property FixedWrapColumn: Integer read FFixedWrapColumn write SetFixedWrapColumn;
  end;
...
begin
  if FFixedWrapColumn > 0 then
    Exit(FFixedWrapColumn);
  Result := TSynEdit(Editor).CharsInWindow - 1;
end;

// meme invalidation que DoWidthChanged: tout re-wrapper a la nouvelle colonne
procedure TLazSynEditLineWrapPlugin.SetFixedWrapColumn(AValue: Integer);
begin
  if AValue < 0 then AValue := 0;
  if FFixedWrapColumn = AValue then Exit;
  FFixedWrapColumn := AValue;
  FLineMapView.KnownLengthOfLongestLine := WrapColumn;
  FLineMapView.InvalidateLines(0, FLineMapView.NextLines.Count);
  if Editor <> nil then
